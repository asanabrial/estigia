...
configuration reads that lead into adapter mutation. Guided setup checks before opening the screen.
The check is global rather than per-adapter because setup batches intentionally continue after an
adapter failure. Dry-run returns before lifecycle inspection because it cannot deploy assets.
