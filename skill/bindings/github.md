...
| `reclaim` | `SCRIPT reclaim --issue <n> --run-id <id> --runtime <rt> --horizon <UTC> --operation-id <32-hex> [--state <s>] [--target-operation <epoch>] [--force --reason-file <f>]` | target discovery is read-only; the write binds target, evidence, privilege and metadata, then proves exact projections. It refuses the publishing/requesting run of an unresolved review handoff on the same terms as `claim`, so a replacement that went stale cannot return the item to the one run forbidden to review it |
| `verify_claim` | `SCRIPT verify-claim --issue <n> --run-id <id> --expect-state <s> [--allow-closed-by-pr <pr>]` | proves the requested run is the reducer's current live winner and uses timeline position, not second-precision timestamps, as its control-message watermark. A closed-issue exception reads every page of closing PRs and matches this run's publication receipt, the closing PR, and a merge whose parents are that receipt's base and head, and answers `closed-by-own-delivery` with the delivering SHA and epoch. `--allow-closed-by-pr` pins that PR. Any other close is still `issue-not-open`. Incomplete or malformed pagination is a failed read |
| `transition` | `SCRIPT transition --issue <n> --to <s> [--from <s>]` | mirrors the board **first**, swaps the label in **one** call, then reads **both** back and repairs a board that disagrees. Omitting `--from` removes whatever stale state labels it finds; `--from` naming the same state as `--to` removes nothing, so asserting the state an item already holds is safe. A read-back that disagrees is refused **as a write that landed**, and the refusal names the repair: the same transition with `--from` omitted |
...
| `review_finding` | `SCRIPT review-finding --issue <n> --run-id <id> --reviewer <id> --epoch <publication> --pr <n> --head <sha> --base <sha> --digest <sha256> --id <lineage-key> --class severe\|warning\|suggestion --evidence <text> --impact <text> [--parent <lineage-key>] [--origin introduced\|exposed]` | requires a live claim in `review` by the recording run, and records **one** observation against **one** exact receipt. `severe` is a reproducible correctness, security, reliability or contract defect with concrete impact; `warning` is a bounded risk with no demonstrated incorrectness; `suggestion` covers optional improvement, cosmetic wording and preference. An unrecognised class is refused rather than defaulted, because a classification nobody chose is how a preference becomes a blocker. A finding with no `--id`, `--evidence` or `--impact` is refused for the same reason: one that cannot be re-run cannot be checked and one with no impact cannot be weighed. `--id` is derived from the affected behaviour and location, never from the run or the wording, so two judges reporting one defect agree rather than counting twice and a repair can name what it answers. The operation is keyed on nine fields — run, reviewer, epoch, PR, head, base, digest, `--id` and `--class` — so a retry after an unreadable answer reads back what it already wrote rather than turning one observation into two confirmations. `--evidence`, `--impact`, `--parent` and `--origin` are deliberately outside that key: re-recording one finding with different wording is the same operation and the first body stands, so correcting the wording means a new `--id`, not a replay. Findings bind to the receipt and are **not** carried across a republish. Every finding must name the publication **under review**; a well-formed receipt that is not the current one is refused with `published-receipt-mismatch`, the same refusal the verdict gives. Across a repair, `--parent` names the finding this one reassesses and must name one the parent **receipt** recorded — not merely one carrying the parent epoch, which any later marker can claim; a `severe` finding naming no parent must state `--origin introduced` or `--origin exposed`, while a warning or a suggestion new to the repair states neither |
| `review_verdict` | `SCRIPT review-verdict --issue <n> --run-id <id> --reviewer <id> --epoch <publication> --pr <n> --head <sha> --base <sha> --digest <sha256> --outcome accepted\|rejected` | requires a live claim in `review` by the recording run, the exact globally latest receipt, and a credited reviewer distinct from the publisher and every requester. No handoff is required: after one the reviewing run names itself, and a run that acquired a reviewer without releasing the claim names that reviewer instead — the answer marks the second `self_attested`, which records a declaration rather than checking it. The immutable marker is operation-idempotent. The latest qualifying outcome resolves any transfer so the publishing run may resume; only `accepted` qualifies delivery, and a later rejection withdraws that qualification. Prose, CI, elapsed deadlines, PR readiness, edited/untrusted markers, stale receipts and the publishing/requesting run resolve nothing. An `accepted` verdict additionally requires the **publication lane for the receipt's own head** to have answered green: check runs are read from `repos/<owner>/<name>/commits/<head>/check-runs`, a failed conclusion refuses `publication-lane-red`, a queued or running one refuses `publication-lane-unfinished`, an unreadable listing is a failed read, and **no check runs at all proceeds** — Estigia cannot prove a repository has a lane. A `rejected` verdict is never gated on it: a rejection over red bytes is the route out, not the waste |
...
| `review-handoff-operation-conflict` / `review-verdict-operation-conflict` / `review-finding-operation-conflict` | One deterministic operation ID is already bound to different or incomplete immutable marker fields. | Do not edit protocol comments. Re-read the timeline; unchanged retries reuse the original event, while a genuinely different event needs different immutable inputs. |
...
| `publication-lane-forbidden` | This token may not start workflows, so `publish_review` could not dispatch the publication lane. The branch is pushed and **no receipt was recorded**. | Grant the permission — `gh auth refresh -s workflow`, or `actions: write` on the token in `GH_TOKEN` — then run the same call again; it reuses the pull request. |
| `rejection-without-severe-finding` | The verdict says `rejected` and this reviewer has recorded no `severe` finding against that exact receipt. A rejection costs a republish, a new epoch and a full re-review, so it rests on a defect rather than on a preference. | Record the severe finding first with `review_finding`, or record `accepted` and keep the observation as a `warning` or a `suggestion` — both are preserved beside an acceptance and neither withdraws it. |
| `invalid-finding-class` | The class is not one of `severe`, `warning` or `suggestion`. Refused rather than defaulted: a classification nobody chose is how a preference becomes a blocker. | Re-record with one of the three. |
| `incomplete-review-finding` | The finding has no `--id`, no `--evidence` or no `--impact`. One that cannot be re-run cannot be checked, and one with no stated impact cannot be weighed. | Supply the missing field, or drop the observation. |
| `invalid-finding-origin` | `--origin` is neither `introduced` nor `exposed`. Those are the two words the vocabulary accepts; a third word is a claim no decision table can weigh. They do not cover a severe defect that was always present and simply missed — that case is recorded in `docs/honesty.md` rather than given a third word here. | Re-record with one of the two, or omit it and name the finding this reassesses instead. |
| `finding-parent-without-lineage` | The finding names a parent, and this publication descends from nothing. A first publication has no earlier ledger for that identity to have come from. | Drop `--parent`. Nothing on this receipt is settled work yet. |
| `finding-parent-unknown` | The named parent is not a finding recorded against the parent **receipt**. An epoch alone would not do: an epoch is not a function of the bytes it names, and a marker written after the repair can carry the parent epoch beside the repair’s own bytes. Left unchecked, `--parent` would be free text that makes a fresh observation read as already settled, which is exactly what a re-review acts on. | The refusal lists the identities the parent receipt recorded. Name one of those, or record this as new to this publication and state its `--origin`. |
...
`.github/workflows/ci.yml` is the example. It also carries `workflow_dispatch:`, which is the
**publication lane**: `publish_review` and `republish_review` start one run of it against the head they
just pushed, once per publication epoch and not once per push, and dispatching does not mark the pull
request ready. `record_review_verdict` then refuses an `accepted` verdict for a head whose lane is red
or unfinished, so the cheapest disqualifying signal is collected before the most expensive resource is
spent rather than after it. A repository whose lane is named otherwise, or has none, gets the old
ordering: Estigia does not parse consumer YAML and cannot prove a lane exists. `publish_review` makes a
reused PR draft and reads that back before push, or creates a new PR with `--draft`; judges can
therefore run before ordinary cooperative CI. `release_ci` replays the live claim, globally latest receipt, draft PR and coherent target, then
