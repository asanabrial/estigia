...
| `review_verdict` | `SCRIPT review-verdict --issue <n> --run-id <reviewer> --epoch <publication> --pr <n> --head <sha> --base <sha> --digest <sha256> --outcome accepted\|rejected` | requires a live claim in `review`, the exact globally latest receipt, a recorded handoff, and a reviewer run distinct from its publisher and every requester. The immutable marker is operation-idempotent. The latest distinct outcome resolves the transfer so the publishing run may resume; only `accepted` qualifies delivery, and a later rejection withdraws that qualification. Prose, CI, elapsed deadlines, PR readiness, edited/untrusted markers, stale receipts and the publishing/requesting run resolve nothing |
...
| `review_status` | *(agent judgement, recorded by `review_verdict`)* | The reviewer judges the change; the tool records only outcome and exact publication identity. Use one independent context for `Blind judges: single`, or the two-context agreement policy for `two blind`; Estigia cannot prove a judge ran, was independent, read the bytes, or was blind. Reject a verdict naming any earlier epoch |
...
| `review-handoff-requester-excluded` / `reviewer-not-distinct` | The publishing/requesting run tried to reclaim or satisfy its own unresolved handoff. | Use another run to claim the review and record the exact-receipt verdict. Time, CI and prose never discharge it. |
| `review-handoff-missing` / `qualifying-review-verdict-missing` | No durable handoff or no distinct accepted verdict matches the latest full receipt. | Keep the PR draft and the issue in `review`; record the missing exact-receipt evidence. |
