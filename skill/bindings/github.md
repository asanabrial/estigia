...
| `invalid-finding-origin` | `--origin` is neither `introduced` nor `exposed`. Those are the only two ways a repair is answerable for a defect that is new to it, and a third word is a claim no decision table can weigh. | Re-record with one of the two, or omit it and name the finding this reassesses instead. |
