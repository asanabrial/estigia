...
| `finding-parent-unknown` | The named parent is not a finding recorded against the parent epoch. Left unchecked, `--parent` would be free text that makes a fresh observation read as already settled, which is exactly what a re-review acts on. | The refusal lists the identities the parent epoch recorded. Name one of those, or record this as new to this publication and state its `--origin`. |
