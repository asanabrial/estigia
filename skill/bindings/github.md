...
Every ownership write uses a fresh lowercase 32-hex `--operation-id`, reused unchanged on retry. `reclaim` and `unassign` first return the exact target without writing; repeat with `--target-operation` to authorize only that epoch. Historical legacy ownership remains readable, but broad legacy controls cannot mutate a modern epoch.
...
| `list_state` | `SCRIPT list-state --state <s>` | unassigned only, `--limit 200` (gh's own default is 30 and silently truncates the queue; 200 is **also a ceiling**, so an answer whose `count` equals the limit may be a longer queue read to its limit, and `count` is what came back rather than what there is — raise `--limit` to find out), partitioned by `domain:<name>`. It returns the raw labels and **refuses to rank across partitions** — ordering inside one needs the domain's scale contract, and manufacturing a global rank is forbidden |
...
| `release_ci` | `SCRIPT release-ci --issue <n> --run-id <id> --epoch <id> --pr <n> --head <sha> --base <sha> --digest <sha256> [--worktree <p>]` | verifies a live claim in `review`, requires the supplied receipt to be the globally latest complete publication and attributed to this run, re-reads the same draft PR/head/base, and re-derives one coherent clean target before `gh pr ready`. Every ready outcome is read back: an exact ready target is success even when the command failed ambiguously; otherwise the read/write taxonomy is preserved |
...
| `review_status` | *(agent, not scripted)* | The independent verdict artifact MUST identify the publication epoch, PR, full head, full base, and target digest. Use one independent context for `Blind judges: single`, or the two-context agreement policy for `two blind`; Estigia records and rechecks the receipt but cannot prove a judge ran, was independent, or was blind. Reject a verdict naming any earlier epoch |
