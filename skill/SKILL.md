...
| State must be chosen | Invalid specification -> `analysis`; implementable and unassigned -> `ready`; claimed build -> `in-progress`; built work awaiting delivery evidence -> `review`; work awaiting a person's decision -> `blocked` with the exact decision or exit condition and discharger; merged and verified -> `done`. |
...
7. The built-work handoff in steps 5 and 6 distinguishes two waits. Missing review, CI, current-head/base evidence, or delivery permission is delivery evidence and stays in `review` with its exact blocker. A contradiction, exhausted convergence budget, or other point that requires a named person's decision goes to `blocked` only after the branch, PR, receipt, checks, evidence, exact decision or exit condition, and discharger are recorded; then release ownership.
