...
| State must be chosen | Invalid specification -> `analysis`; implementable and unassigned -> `ready`; claimed build -> `in-progress`; built work awaiting delivery evidence or ordinary delivery permission -> `review`; exceptional human adjudication outside ordinary delivery gates -> `blocked` with the exact decision or exit condition and discharger; merged and verified -> `done`. |
