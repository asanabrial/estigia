...
- A claimed task reaches `done` or a state that names its exact exit condition and discharger; it never enters an ownerless, conditionless queue.
- The selected binding MUST map `ensure_states`, `create`, `list_state`, `claim`, `reclaim`, `verify_claim`, `heartbeat`, `transition`, `comment`, `last_activity`, `label`, `unassign`, `publish_review`, `republish_review`, `handoff_review`, `review_finding`, `review_verdict`, `release_ci`, `publish_version`, and `close`. Run executable reversible operations instead of reconstructing them; bindings MUST declare unsupported capabilities and fail closed.
...
| State must be chosen | Invalid specification -> `analysis`; implementable and unassigned -> `ready`; claimed build -> `in-progress`; built work awaiting delivery evidence or ordinary delivery permission -> `review`; an unbuilt external wait, or exceptional human adjudication outside ordinary delivery gates -> `blocked` with the exact condition or decision and its discharger; merged and verified -> `done`. |
...
5. Renew before publishing. Publish or reuse the shared PR as a confirmed draft and record its exact epoch, PR, head, base, and coherent clean-target digest, then transition to `review`. Acquire the configured single independent reviewer, two blind judges, or five blind judges only when `Review delegation` authorises this run; `auto` permits acquisition but does not create a capability the runtime lacks. A host that can spawn a subagent already has the capability; the Claude-only review-blind type is a launch mechanism, not the capability. Every route ends in the same evidence: one accepted aggregate `review_verdict` naming the exact receipt and crediting a reviewer that is neither the publishing run nor any run that asked for review. Before that verdict, every observation worth carrying is its own `review_finding` against the same receipt, classified `severe`, `warning` or `suggestion` with concrete evidence and stated impact. A finding naming any receipt but the publication under review is refused. A `rejected` verdict is refused unless that reviewer recorded a `severe` finding against that exact receipt; with no severe finding the outcome is `accepted`, and warnings and suggestions are preserved beside it without withdrawing it. Cosmetic, style and preference observations are `suggestion` unless they identify a concrete defect. If concrete evidence and material impact cannot both be stated, omit the observation or keep it as a non-blocking note. When a panel produced the verdict, record each judge's findings under that judge's identity and credit the aggregate verdict to a judge whose severe finding it rests on. A publication over an earlier one records the whole parent receipt and a delta digest, derived from the timeline rather than supplied; a finding that reassesses one the parent ledger holds names it — and the name must exist against the parent **receipt**, not merely against its epoch — while one that is `severe` and new to the repair states whether the repair `introduced` or `exposed` it. A run that acquired its own reviewer records that verdict itself, keeping the claim, and the answer marks it `self_attested` — Estigia has recorded the declaration, not checked it. When this run cannot obtain the required distinct context — the launch failed, or the runtime cannot spawn a helper — call `handoff_review` once with the exact receipt, current ownership epoch, blocker, and discharger. It records and reads back one immutable handoff, then releases only that ownership epoch while retaining `review`; the same publishing/requesting run cannot select or reclaim it until a distinct reviewer records the verdict. That reviewer claims it, reviews, records the verdict, and unassigns so the publishing run can resume. A rejected verdict resolves the transfer so the author can fix and republish, but never qualifies delivery; only an accepted latest verdict releases CI. Delivery asks for that verdict on every route, so no deletion can manufacture evidence that was never recorded; `docs/honesty.md` names the narrower cases where deleting a marker restores evidence a later one had disqualified. A timed `ask` records one deadline but never sleeps, schedules work, resets the deadline, retains ownership, or treats expiry as a verdict. Estigia cannot prove panel size, concurrency, independence, blindness, same-finding identity or quorum. After the qualifying verdict evidence names that exact receipt, call `release_ci`; it re-verifies the live review claim, globally latest recorded receipt across runs, distinct accepted verdict, current draft PR, and re-derived coherent clean target before making the PR ready, then reads back every outcome. Every republish creates a new epoch and invalidates prior review evidence even when the bytes are identical. A republish whose history was rewritten — rebased onto a moved base, or amended — is `republish_review` and never a hand-run force-push: it leases against the head the last publication recorded and refuses when the remote moved since, so the one step that destroys history is the one step still adjudicated. `publish_review` never force-pushes, so a non-fast-forward refusal there is the signal to use it rather than to leave. This ordering is cooperative: the repository must run PR CI on `ready_for_review` only, not topic push/open/synchronize. Beside it, `publish_review` and `republish_review` start the **publication lane** — one `workflow_dispatch` run against the head they just pushed, once per publication epoch and never per push, and it does not mark the PR ready — and `record_review_verdict` refuses an accepted verdict for a head whose lane is red or still running, so a platform failure the reviewers could not see is found before their verdicts are spent rather than after. A head with no check runs at all proceeds: Estigia starts the lane and cannot prove a repository has one. GitHub has no atomic conditional-ready operation, and collaborators or repository workflows acting outside Estigia can mark ready, push, start CI, or forge evidence; Estigia cannot prove judges ran or authenticate against malicious collaborators. Leave blocked delivery in `review`, restore local state changed by the failed attempt, record the precise blocker, and use the durable handoff rather than bypassing it; conflicting repository rules get a separate issue.
   **Blind-panel clarification for step 5.** `five blind` is canonical beside `single` and `two blind`.
   ...
   over the identical exact receipt and criteria, with no sibling-output access. In five-blind mode,
   3-of-5 must independently confirm the same severe finding before it blocks or authorizes automatic
   repair; one or two confirmations remain suspicions, ambiguous identities do not aggregate, and
   ...
   any project `.claude/agents` collision found through parsed YAML from the launch cwd through the
   first `.git` repository root, or any duplicate or unprovable definition in the canonical user tree,
   refuses the launch; a running reviewer is gated by the embedded policy, never project bytes. No
   refused or unprovable judge counts. Use a separate session or durable handoff when the reserved
   launch cannot be restored; never reduce or serialize the configured panel.
6. Renew at each irreversible boundary; merge only when reviewed, green, and current head/base identities match. Deliver on your own only when `Delivery authorisation` authorises this run; otherwise ask first, with the reviewed head and base named. For a merge-commit route, require the delivered SHA's exact parents to be the reviewed base and head; other configured strategies require their binding-declared equivalent topology and any delivered-SHA gate. For each changed version, publish and remotely verify its immutable annotated tag and required Release; renew again, transition to `done`, record review/CI/tests/measurements/PR/delivering SHA/publications with the run-id, and close. On handoff, preserve all evidence: wrong specification goes to `analysis`, unbuilt external wait to `blocked`, built work awaiting delivery evidence to `review`, and a useful partial diagnosis to `ready`.

7. The built-work handoff in steps 5 and 6 distinguishes two waits. Missing review, CI, current-head/base evidence, or delivery permission is delivery evidence and stays in `review` with its exact blocker, even when a named person must supply the permission. A contradiction in that review evidence — judges disagreeing about the same bytes — an exhausted convergence budget, or other exceptional adjudication outside those ordinary delivery gates goes to `blocked` only after the branch, PR, receipt, checks, evidence, exact decision or exit condition, and discharger are recorded; then release ownership. A contradiction or an unresolved choice about what should be built is not a built-work wait at all: the decision gate routes it to `analysis`.
...
| [blind judges](policies/blind-judges.md) | Independent review is obtained under `Blind judges: two blind` or `Blind judges: five blind`, which are the modes that route it. `single` routes no policy document; its whole decision rule is in developer step 5 above, and issue #18 owns that reachability gap. |
