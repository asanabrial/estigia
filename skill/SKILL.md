...
7. The built-work handoff in steps 5 and 6 distinguishes two waits. Missing review, CI, current-head/base evidence, or delivery permission is delivery evidence and stays in `review` with its exact blocker, even when a named person must supply the permission. A contradiction, exhausted convergence budget, or other exceptional adjudication outside those ordinary delivery gates goes to `blocked` only after the branch, PR, receipt, checks, evidence, exact decision or exit condition, and discharger are recorded; then release ownership.
