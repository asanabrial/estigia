...
4. **Run everything**: `cargo test`, `cargo clippy --all-targets`, `cargo fmt`, `cargo doc
   --no-deps`. One test needs `node` on the path:
   `the_plugin_hands_the_gate_the_directory_the_call_runs_in`, which *executes* the OpenCode plugin
   this crate generates, because that plugin is JavaScript and every other test of it reads its
   source text. It does not skip when `node` is missing; it fails and says so, since a skip spelled
   as a pass is a defect this repository has filed against itself. Nothing else here asks for an
   interpreter, and neither does `doctor`.
